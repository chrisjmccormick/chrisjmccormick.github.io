...
For linear regression, we have a linear hypothesis function, \\( h \left (  x  \right ) = \theta_{0} + \theta_{1}x \\). We want to find the values of \\({\theta}_{0} \\) and \\({\theta}_{1} \\) which provide the best fit of our hypothesis to a training set. The training set examples are labeled x, y, where x is the input value and y is the output. The ith training example is labeled as \\( x^{(1)} \\), \\( y^{(1)} \\). Do not confuse this as an exponent! It just means that this is the ith training example.
